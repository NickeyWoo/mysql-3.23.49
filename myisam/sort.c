...
static ha_rows NEAR_F 
find_all_keys(MI_SORT_PARAM * info, uint keys,
	      uchar ** sort_keys,
	      BUFFPEK * buffpek, int *maxbuffer,
	      IO_CACHE * tempfile);
static int NEAR_F 
write_keys(MI_SORT_PARAM * info, uchar * *sort_keys,
	   uint count, BUFFPEK * buffpek, IO_CACHE * tempfile);
static int NEAR_F 
write_index(MI_SORT_PARAM * info, uchar * *sort_keys,
	    uint count);
static int NEAR_F 
merge_many_buff(MI_SORT_PARAM * info, uint keys,
		uchar * *sort_keys,
		BUFFPEK * buffpek, int *maxbuffer,
		IO_CACHE * t_file);
static uint NEAR_F 
read_to_buffer(IO_CACHE * fromfile, BUFFPEK * buffpek,
	       uint sort_length);
static int NEAR_F 
merge_buffers(MI_SORT_PARAM * info, uint keys,
	      IO_CACHE * from_file, IO_CACHE * to_file,
	      uchar * *sort_keys, BUFFPEK * lastbuff,
	      BUFFPEK * Fb, BUFFPEK * Tb);
static int NEAR_F 
merge_index(MI_SORT_PARAM *, uint, uchar **, BUFFPEK *, int,
	    IO_CACHE *);
...
int 
_create_index_by_sort(MI_SORT_PARAM * info, my_bool no_messages,
		      ulong sortbuff_size)
{
	...
	(*info->lock_in_memory) (info->sort_info->param);	/* Everything is
								 * allocated */
	      ...
	      int *maxbuffer, IO_CACHE * tempfile)
{
	...
	if (error > 0)
		DBUG_RETURN(HA_POS_ERROR);	/* Aborted by get_key *//* pur
						 * ecov: inspected */
	if (indexpos)
		if (indexpos >= (uint) * maxbuffer ||
...
static int NEAR_F 
write_keys(MI_SORT_PARAM * info, register uchar ** sort_keys,
...
static int NEAR_F 
write_index(MI_SORT_PARAM * info, register uchar ** sort_keys,
	    register uint count)
{
		...
		for (;;) {
			...
			if (!--buffpek->mem_count) {
				if (!(error = (int)read_to_buffer(from_file, buffpek, sort_length))) {
					...
					/*
					 * Put room used by buffer to use in
					 * other buffer
					 */
					...
					break;	/* One buffer have been
						 * removed */
				}
			} else if (error == -1)
				goto err;	/* purecov: inspected */
			queue_replaced(&queue);	/* Top element has been
						 * replaced */
		}
	}
	buffpek = (BUFFPEK *) queue_top(&queue);
...
static char   **
make_char_array(register uint fields, uint length, myf my_flag)
{
	register char **pos;
