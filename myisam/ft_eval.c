...
get_options(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, options)) != -1) {
		...
		case 'q':
			...
			break;
		case 'S':
			if (stopwordlist == ft_precompiled_stopwords)
				stopwordlist = NULL;
			break;
		case '#':
			DEBUGGER_ON;
	...
	if (!(d_file = argv[optind]))
		print_error(1, "No d_file");
	if (!(df = fopen(d_file, "r")))
		print_error(1, "fopen(%s)", d_file);
	if (!(q_file = argv[optind + 1]))
		print_error(1, "No q_file");
...
static int 
create_record(char *pos, FILE * file)
{
	uint		tmp;
	char           *ptr;
...
static void 
print_error(int exit_code, const char *fmt,...)
{
	va_list		args;
