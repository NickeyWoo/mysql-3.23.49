...
/*
 * * Make a intern key from a record * Ret: Length of key
 */
...
uint 
_mi_pack_key(register MI_INFO * info, uint keynr, uchar * key, uchar * old,
	     uint k_length)
{
	uint		length;
...
static int 
_mi_put_key_in_record(register MI_INFO * info, uint keynr,
		      byte * record)
{
	reg2 byte      *key;
	byte           *pos, *key_end;
...
int 
_mi_read_key_record(MI_INFO * info, my_off_t filepos, byte * buf)
{
	...
	if (filepos != HA_OFFSET_ERROR) {
		if (info->lastinx >= 0) {	/* Read only key */
			...
			info->update |= HA_STATE_AKTIV;	/* We should find a
							 * record */
			return 0;
		}
...
void 
update_auto_increment(MI_INFO * info, const byte * record)
{
	ulonglong	value;
