...
static ha_rows 
_mi_record_pos(MI_INFO * info, const byte * key, uint key_len,
	       enum ha_rkey_function search_flag);
static double 
_mi_search_pos(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * key,
	       uint key_len, uint nextflag, my_off_t pos);
static uint 
_mi_keynr(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * page,
...
ha_rows 
mi_records_in_range(MI_INFO * info, int inx, const byte * start_key,
		    uint start_key_len,
...
static double 
_mi_search_pos(register MI_INFO * info,
	       ...
	       register my_off_t pos)
{
	...
	if (flag) {
		 ...
		 * Try to find a smaller, better matching key. * Matches
		 * keynr + [0-1]
		 */
		if (flag > 0 && !nod_flag)
			offset = 1.0;
		...
		((keyinfo->flag & (HA_NOSAME | HA_NULL_PART)) != HA_NOSAME ||
		 key_len) && nod_flag) {
			/*
			 * * There may be identical keys in the tree. Try to
			 * match on of those. * Matches keynr + [0-1]
			 */
...
static uint 
_mi_keynr(MI_INFO * info, register MI_KEYDEF * keyinfo, uchar * page, uchar * keypos, uint * ret_max_key)
{
