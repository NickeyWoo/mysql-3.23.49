...
static void 
read_huff_table(MI_BIT_BUFF * bit_buff, MI_DECODE_TREE * decode_tree,
		uint16 ** decode_table, byte ** intervall_buff,
		uint16 * tmp_buff);
static void 
make_quick_table(uint16 * to_table, uint16 * decode_table,
		 uint * next_free, uint value, uint bits,
		 uint max_bits);
static void 
fill_quick_table(uint16 * table, uint bits, uint max_bits,
		 uint value);
static uint 
copy_decode_table(uint16 * to_pos, uint offset,
		  uint16 * decode_table);
static uint	find_longest_bitstream(uint16 * table);
static void     (*
	     get_unpack_function(MI_COLUMNDEF * rec)) (MI_COLUMNDEF * field,
						       MI_BIT_BUFF * buff,
						       uchar * to,
						       uchar * end);
	...
	my_bool		_mi_read_pack_info(MI_INFO * info, pbool fix_keys)
{
	...
	if (bit_buff.error || bit_buff.pos < bit_buff.end) {	/* info_length was wrong */
		my_errno = HA_ERR_WRONG_IN_RECORD;
		my_free((gptr) share->decode_trees, MYF(0));
		my_free((gptr) share->decode_tables, MYF(0));
		DBUG_RETURN(1);
...
static uint 
find_longest_bitstream(uint16 * table)
{
...
int 
_mi_read_pack_record(MI_INFO * info, my_off_t filepos, byte * buf)
{
...
int 
_mi_pack_rec_unpack(register MI_INFO * info, register byte * to, byte * from,
		    ulong reclength)
{
	byte           *end_field;
...
static void     (*
		 get_unpack_function(MI_COLUMNDEF * rec))
...
static void 
uf_zerofill_skipp_zero(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff,
		       uchar * to, uchar * end)
{
	if (get_bit(bit_buff))
...
static void 
uf_skipp_zero(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff, uchar * to,
	      uchar * end)
{
	if (get_bit(bit_buff))
		bzero((char *)to, (uint) (end - to));
...
static void 
uf_space_normal(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff, uchar * to,
		uchar * end)
{
	if (get_bit(bit_buff))
...
static void 
uf_space_endspace_selected(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff,
			   uchar * to, uchar * end)
{
	uint		spaces;
	if (get_bit(bit_buff))
...
static void 
uf_endspace_selected(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff,
		     uchar * to, uchar * end)
{
	uint		spaces;
	if (get_bit(bit_buff)) {
...
static void 
uf_space_endspace(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff, uchar * to,
		  uchar * end)
{
	uint		spaces;
	if (get_bit(bit_buff))
...
static void 
uf_endspace(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff, uchar * to,
	    uchar * end)
{
	uint		spaces;
	if ((spaces = get_bits(bit_buff, rec->space_length_bits)) + to > end) {
...
static void 
uf_space_prespace_selected(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff,
			   uchar * to, uchar * end)
{
	uint		spaces;
	if (get_bit(bit_buff))
...
static void 
uf_prespace_selected(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff,
		     uchar * to, uchar * end)
{
	uint		spaces;
	if (get_bit(bit_buff)) {
...
static void 
uf_space_prespace(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff, uchar * to,
		  uchar * end)
{
	uint		spaces;
	if (get_bit(bit_buff))
...
static void 
uf_prespace(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff, uchar * to,
	    uchar * end)
{
	uint		spaces;
	if ((spaces = get_bits(bit_buff, rec->space_length_bits)) + to > end) {
...
static void 
uf_zerofill_normal(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff, uchar * to,
		   uchar * end)
{
	end -= rec->space_length_bits;
...
static void 
uf_intervall(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff, uchar * to,
	     uchar * end)
{
...
static void 
uf_blob(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff,
	uchar * to, uchar * end)
{
	if (get_bit(bit_buff))
...
static void 
uf_varchar(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff,
...
static void 
decode_bytes(MI_COLUMNDEF * rec, MI_BIT_BUFF * bit_buff, uchar * to,
	     uchar * end)
{
	...
	do {
		...
		if (low_byte & IS_CHAR) {
			*to++ = (low_byte & 255);	/* Found char in quick
							 * table */
...
static uint 
decode_pos(MI_BIT_BUFF * bit_buff, MI_DECODE_TREE * decode_tree)
{
...
int 
_mi_read_rnd_pack_record(MI_INFO * info, byte * buf,
...
uint 
_mi_pack_get_block_info(MI_INFO * myisam, MI_BLOCK_INFO * info, File file,
			my_off_t filepos, char *rec_buff)
{
	...
	if (file >= 0) {
		...
		/*
		 * We can't use my_pread() here because
		 * mi_read_rnd_pack_record assumes position is ok
		 */
...
static void 
init_bit_buffer(MI_BIT_BUFF * bit_buff, uchar * buffer, uint length)
{
	bit_buff->pos = buffer;
...
static uint 
fill_and_get_bits(MI_BIT_BUFF * bit_buff, uint count)
{
	uint		tmp;
	count -= bit_buff->bits;
...
static void 
fill_buffer(MI_BIT_BUFF * bit_buff)
{
...
static uchar   *
_mi_mempack_get_block_info(MI_INFO * myisam, MI_BLOCK_INFO * info,
			   uchar * header)
{
	if (header[0] < 254)
		info->rec_len = *header++;
...
static int 
_mi_read_mempack_record(MI_INFO * info, my_off_t filepos, byte * buf)
{
	MI_BLOCK_INFO	block_info;
	MYISAM_SHARE   *share = info->s;
	byte           *pos;
...
/* ARGSUSED */
static int 
_mi_read_rnd_mempack_record(MI_INFO * info, byte * buf,
			    register my_off_t filepos,
			    my_bool skip_deleted_blocks
...
uint 
save_pack_length(byte * block_buff, ulong length)
{
	if (length < 254) {
