...
int 
mi_update(register MI_INFO * info, const byte * oldrec, byte * newrec)
{
	...
	key_changed = HA_STATE_KEY_CHANGED;	/* We changed current
						 * database */
		...
		if (((ulonglong) 1 << i) & share->state.key_map) {
			...
			if (share->keyinfo[i].flag & HA_FULLTEXT) {
				...
				if (new_length != old_length ||
					...
					if ((int)i == info->lastinx)
						key_changed |= HA_STATE_WRITTEN;	/* Mark that keyfile
											 * changed */
					changed |= ((ulonglong) 1 << i);
					share->keyinfo[i].version++;
					if (_mi_ck_delete(info, i, old_key, old_length))
						goto err;
					if (_mi_ck_write(info, i, new_key, new_length))
						goto err;
