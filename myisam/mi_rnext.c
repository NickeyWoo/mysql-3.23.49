...
/*
 * Read next row with the same key as previous read One may have done a
 * write, update or delete of the previous row. NOTE! Even if one changes the
 * previous row, the next read is done based on the position of the last used
 * key!
 */

int 
mi_rnext(MI_INFO * info, byte * buf, int inx)
{
	int		error     , changed;
	uint		flag;
	DBUG_ENTER("mi_rnext");
