...
/*
 * locking of isam-tables. reads info from a isam-table. Must be first
 * request before doing any furter calls to any isamfunktion.  Is used to
 * allow many process use the same isamdatabase.
 */
...
int 
mi_lock_database(MI_INFO * info, int lock_type)
{
	int		error;
	uint		count;
	...
	if (share->kfile >= 0) {/* May only be false on windows */
		...
		case F_UNLCK:
			...
			if (info->lock_type == F_WRLCK && !share->w_locks &&
				...
				error = my_errno;
				mi_mark_crashed(info);	/* Mark that table must
							 * be checked */
			}
			...
			break;
		case F_RDLCK:
			if (info->lock_type == F_WRLCK) {	/* Change RW to READONLY */
				if (share->w_locks == 1) {
					flag = 1;
...
void 
mi_update_status(void *param)
{
	MI_INFO        *info = (MI_INFO *) param;
	/*
	 * Because someone may have closed the table we point at, we only
	 * update the state if its our own state.  This isn't a problem as we
	 * are always pointing at our own lock or at a read lock. (This is
	 * enforced by thr_multi_lock.c)
	 */
...
void 
mi_copy_status(void *to, void *from)
{
	((MI_INFO *) to)->state = &((MI_INFO *) from)->save_state;
}

my_bool 
mi_check_status(void *param)
{
	MI_INFO        *info = (MI_INFO *) param;
...
int 
_mi_readinfo(register MI_INFO * info, int lock_type, int check_keybuffer)
{
	MYISAM_SHARE   *share;
...
int 
_mi_writeinfo(register MI_INFO * info, uint operation)
{
	int		error     , olderror;
	MYISAM_SHARE   *share = info->s;
...
int 
_mi_test_if_changed(register MI_INFO * info)
{
	MYISAM_SHARE   *share = info->s;
...
int 
_mi_mark_file_changed(MI_INFO * info)
{
	char		buff      [3];
...
/*
 * This is only called by close or by extra(HA_FLUSH) if the OS has the
 * pwrite() call.  In these context the following code should be safe!
 */

int 
_mi_decrement_open_count(MI_INFO * info)
{
	char		buff      [2];
	register MYISAM_SHARE *share = info->s;
