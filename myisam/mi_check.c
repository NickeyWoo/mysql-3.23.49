...
static int 
chk_index(MI_CHECK * param, MI_INFO * info, MI_KEYDEF * keyinfo,
	  my_off_t page, uchar * buff, ha_rows * keys,
	  ha_checksum * key_checksum, uint level);
static uint	isam_key_length(MI_INFO * info, MI_KEYDEF * keyinfo);
static ha_checksum calc_checksum(ha_rows count);
static int 
writekeys(MI_CHECK * param, MI_INFO * info, byte * buff,
	  my_off_t filepos);
static int 
sort_one_index(MI_CHECK * param, MI_INFO * info, MI_KEYDEF * keyinfo,
...
static my_off_t 
get_record_for_key(MI_INFO * info, MI_KEYDEF * keyinfo,
		   uchar * key);
static int 
sort_insert_key(MI_CHECK * param, reg1 SORT_KEY_BLOCKS * key_block,
		uchar * key, my_off_t prev_block);
static int	sort_delete_record(MI_CHECK * param);
static int	flush_pending_blocks(MI_CHECK * param);
static SORT_KEY_BLOCKS *
alloc_key_blocks(MI_CHECK * param, uint blocks,
		 uint buffer_length);
static void 
update_key_parts(MI_KEYDEF * keyinfo,
		 ulong * rec_per_key_part,
		 ulonglong * unique,
		 ulonglong records);
static ha_checksum mi_byte_checksum(const byte * buf, uint length);
static void 
set_data_file_type(MI_CHECK * param, SORT_INFO * info,
		   MYISAM_SHARE * share);
...
int 
chk_status(MI_CHECK * param, register MI_INFO * info)
{
	...
	if (share->state.open_count != (uint) (info->s->global_changed ? 1 : 0)) {
		/*
		 * Don't count this as a real warning, as check can correct
		 * this !
		 */
...
int 
chk_del(MI_CHECK * param, register MI_INFO * info, uint test_flag)
{
	reg2 ha_rows	i;
...
static int 
check_k_link(MI_CHECK * param, register MI_INFO * info, uint nr)
{
	my_off_t	next_link;
...
int 
chk_size(MI_CHECK * param, register MI_INFO * info)
{
	int		error = 0;
	...
	if (skr != size) {
		...
		if (skr > size && skr != size + MEMMAP_EXTRA_MARGIN) {
			...
			mi_check_print_error(param, "Size of datafile is: %-9s         Should be: %s",
				      llstr(size, buff), llstr(skr, buff2));
			param->retry_without_quick = 1;	/* Don't use quick
							 * repair */
		} else {
			mi_check_print_warning(param,
...
int 
chk_key(MI_CHECK * param, register MI_INFO * info)
{
	...
	for (key = 0, keyinfo = &share->keyinfo[0]; key < share->base.keys;
		...
		if ((uint) share->base.auto_key - 1 == key) {
			/*
			 * Check that auto_increment key is bigger than max
			 * key value
			 */
			...
			/*
			 * Check that there isn't a row with auto_increment =
			 * 0 in the table
			 */
			...
			if (!mi_rkey(info, info->rec_buff, key, (const byte *)info->lastkey,
				...
				/*
				 * Don't count this as a real warning, as
				 * myisamchk can't correct it
				 */
				uint		save = param->warning_printed;
				mi_check_print_warning(param,
	  ...
	  ha_checksum * key_checksum, uint level)
{
	...
	uchar		key      [MI_MAX_POSSIBLE_KEY_BUFF], *temp_buff, *keypos,
	               *old_keypos, *endpos;
...
static uint 
isam_key_length(MI_INFO * info, register MI_KEYDEF * keyinfo)
{
	uint		length;
	MI_KEYSEG      *keyseg;
...
int 
chk_data_link(MI_CHECK * param, MI_INFO * info, int extend)
{
	int		error     , got_error, flag;
	uint		key      , left_length, b_type, field;
	ha_rows		records, del_blocks;
	my_off_t	used  , empty, pos, splits, start_recpos, del_length,
			link_used    , start_block;
	byte           *record, *to;
...
int 
mi_repair(MI_CHECK * param, register MI_INFO * info,
	  my_string name, int rep_quick)
{
	 ...
	 * touch key_map it cannot be used to T_CREATE_MISSING_KEYS. That is
	 * the next line for... (serg)
	 */
...
static int 
writekeys(MI_CHECK * param, register MI_INFO * info, byte * buff,
	  my_off_t filepos)
{
	register uint	i;
	uchar          *key;
...
int 
movepoint(register MI_INFO * info, byte * record, my_off_t oldpos,
...
void 
lock_memory(MI_CHECK * param __attribute__((unused)))
{
...
int 
flush_blocks(MI_CHECK * param, File file)
{
...
int 
mi_sort_index(MI_CHECK * param, register MI_INFO * info, my_string name)
{
	reg2 uint	key;
	...
	for (key = 0, keyinfo = &share->keyinfo[0]; key < share->base.keys;
		...
		if (share->state.key_root[key] != HA_OFFSET_ERROR) {
			index_pos[key] = param->new_file_pos;	/* Write first block
								 * here */
...
int 
change_to_newfile(const char *filename, const char *old_ext,
		  const char *new_ext,
...
int 
lock_file(MI_CHECK * param, File file, my_off_t start, int lock_type,
	  const char *filetype, const char *filename)
{
	if (my_lock(file, lock_type, start, F_TO_EOF,
		...
		param->error_printed = 2;	/* Don't give that data is
						 * crashed */
		return 1;
	}
	return 0;
...
int 
filecopy(MI_CHECK * param, File to, File from, my_off_t start,
...
int 
mi_repair_by_sort(MI_CHECK * param, register MI_INFO * info,
		  const char *name, int rep_quick)
{
	 ...
	 * implies "one row - one key per keynr", while for ft_key one
	 * row/keynr can produce as many keys as the number of unique words
	 * in the text that's why I disabled repair_by_sort for ft-keys.
	 * (serg)
	 */
	...
	if (!(param->testflag & T_CREATE_MISSING_KEYS)) {
		/*
		 * Flush key cache for this file if we are calling this
		 * outside myisamchk
		 */
		flush_key_blocks(share->kfile, FLUSH_IGNORE_CHANGED);
	...
	rec_per_key_part += sort_info->keyinfo->keysegs, sort_info->key++) {
		...
		if (sort_info->fix_datafile) {
			...
			if (param->testflag & T_SAFE_REPAIR) {
				/*
				 * Don't repair if we loosed more than one
				 * row
				 */
				if (info->state->records + 1 < start_records) {
					info->state->records = start_records;
...
static int 
sort_key_read(SORT_INFO * sort_info, void *key)
{
	int		error;
	MI_INFO        *info;
...
static int 
sort_get_next_record(SORT_INFO * sort_info)
{
	int		searching;
		...
		for (;;) {
			...
			do {
				...
				if (searching && !sort_info->fix_datafile) {
					...
					param->retry_repair = param->retry_without_quick = 1;
					DBUG_RETURN(1);	/* Something wrong with
							 * data */
				}
				if (((b_type = _mi_get_block_info(&block_info, -1, pos)) &
					...
					searching = 1;
					/*
					 * Search after block in read header
					 * string
					 */
...
int 
sort_write_record(SORT_INFO * sort_info)
{
	int		flag;
	uint		length;
	...
	if (sort_info->fix_datafile) {
		...
		case DYNAMIC_RECORD:
			if (!info->blobs)
				...
				/*
				 * must be sure that local buffer is big
				 * enough
				 */
...
static int 
sort_key_cmp(SORT_INFO * sort_info, const void *a, const void *b)
{
	uint		not_used;
...
static int 
sort_key_write(SORT_INFO * sort_info, const void *a)
{
	uint		diff_pos;
	char		llbuff    [22], llbuff2[22];
	MI_CHECK       *param = sort_info->param;
...
static int 
sort_insert_key(MI_CHECK * param,
...
static int 
sort_delete_record(MI_CHECK * param)
{
	uint		i;
	int		old_file  , error;
	uchar          *key;
...
static int 
flush_pending_blocks(MI_CHECK * param)
{
	...
	for (key_block = sort_info->key_block; key_block->inited; key_block++) {
		...
		/*
		 * If we read the page from the key cache, we have to write
		 * it back
		 */
		if (key_file_length == info->state->key_file_length) {
			if (_mi_write_keypage(info, sort_info->keyinfo, filepos,
...
int 
test_if_almost_full(MI_INFO * info)
{
	if (info->s->options & HA_OPTION_COMPRESS_RECORD)
		return 0;
...
int 
recreate_table(MI_CHECK * param, MI_INFO ** org_info, char *filename)
{
	int		error;
	MI_INFO		info;
	MYISAM_SHARE	share;
	...
	/*
	 * Copy the unique definitions and change them to point at the new
	 * key segments
	 */
...
int 
write_data_suffix(MI_CHECK * param, MI_INFO * info)
{
	if (info->s->options & HA_OPTION_COMPRESS_RECORD &&
...
int 
update_state_info(MI_CHECK * param, MI_INFO * info, uint update)
{
	MYISAM_SHARE   *share = info->s;
...
/*
 * Update auto increment value for a table When setting the 'repair_only'
 * flag we only want to change the old auto_increment value if its wrong
 * (smaller than some given key). The reason is that we shouldn't change the
 * auto_increment value for a table without good reason when only doing a
 * repair; If the user have inserted and deleted rows, the auto_increment
 * value may be bigger than the biggest current row and this is ok.
 * 
...
void 
update_auto_increment_key(MI_CHECK * param, MI_INFO * info,
			  my_bool repair_only)
{
...
ha_checksum 
mi_byte_checksum(const byte * buf, uint length)
{
	ha_checksum	crc;
...
/*
 * Deactive all not unique index that can be recreated fast These include
 * packed keys on which sorting will use more temporary space than the max
 * allowed file length or for which the unpacked keys will take much more
 * space than packed keys. Note that 'rows' may be zero for the case when we
 * don't know how many rows we will put into the file.
 */

static my_bool 
mi_too_big_key_for_sort(MI_KEYDEF * key, ha_rows rows)
{
...
void 
mi_disable_non_unique_index(MI_INFO * info, ha_rows rows)
{
	MYISAM_SHARE   *share = info->s;
	uint		i;
...
/*
 * Return TRUE if we can use repair by sorting One can set the force argument
 * to force to use sorting even if the temporary file would be quite big!
 */

my_bool 
mi_test_if_sort_rep(MI_INFO * info, ha_rows rows,
		    ulonglong key_map,
		    my_bool force)
{
	MYISAM_SHARE   *share = info->s;
	...
	for (i = 0; i < share->base.keys; i++, key++) {
		/*
		 * It's to disable repair_by_sort for ft-keys. Another
		 * solution would be to make ft-keys just
		 * too_big_key_for_sort, but then they won't be disabled by
		 * dectivate_non_unique_index and so they will be created at
		 * the first stage. As ft-key creation is very time-consuming
		 * process, it's better to leave it to repair stage but this
		 * repair shouldn't be repair_by_sort (serg)
		 */
