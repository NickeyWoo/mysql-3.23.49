	...
	enum ha_rkey_function search_flag)
{
	...
	if (!_mi_search(info, info->s->keyinfo + inx, key_buff, pack_key_length,
		...
		while (info->lastpos >= info->state->data_file_length) {
			 ...
			 * we got a lock Note that this can only happen if we
			 * are not searching after an exact key, because the
			 * keys are sorted according to position
			 */
	...
	if (search_flag == HA_READ_AFTER_KEY)
		info->update |= HA_STATE_NEXT_FOUND;	/* Previous gives last
							 * row */
err:
	DBUG_RETURN(my_errno);
}				/* _mi_rkey */
