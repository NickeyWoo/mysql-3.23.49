 ...
 * ftp://ftp.cs.cornell.edu/pub/smart/smart.11.0.tar.Z NORM_PIVOT was taken
 * from the article A.Singhal, C.Buckley, M.Mitra, "Pivoted Document Length
 * Normalization", ACM SIGIR'96, 21-29, 1996
 */
...
#ifdef EVAL_RUN
/*
 * extern ulong collstat; #define PIVOT_STAT  (docstat.uniq) #define
 * PIVOT_SLOPE (0.69) #define PIVOT_PIVOT
 * ((double)collstat/(info->state->records+1)) #define NORM_PIVOT
 * ((1-PIVOT_SLOPE)*PIVOT_PIVOT+PIVOT_SLOPE*docstat.uniq)
 */
#endif				/* EVAL_RUN */
