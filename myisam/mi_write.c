...
static int 
w_search(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * key,
	 uint key_length, my_off_t pos, uchar * father_buff,
	 uchar * father_keypos, my_off_t father_page,
	 my_bool insert_last);
static int 
_mi_balance_page(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * key,
		 uchar * curr_buff, uchar * father_buff,
		 uchar * father_keypos, my_off_t father_page);
static uchar   *
_mi_find_last_pos(MI_KEYDEF * keyinfo, uchar * page,
...
int 
_mi_ck_write(register MI_INFO * info, uint keynr, uchar * key,
	     uint key_length)
{
	int		error;
...
/*
 * Search after a position for a key and store it there Returns -1 = error 0
 * = ok 1  = key should be stored in higher tree
 */
...
int 
_mi_split_page(register MI_INFO * info, register MI_KEYDEF * keyinfo,
...
/*
 * Calculate how to much to move to split a page in two Returns pointer to
 * start of key. key will contain the key. return_key_length will contain the
 * length of key after_key will contain the position to where the next key
 * starts
 */
...
/*
 * Split buffer at last key Returns pointer to the start of the key before
 * the last key key will contain the last key
 */
...
static int 
_mi_balance_page(register MI_INFO * info, MI_KEYDEF * keyinfo,
		 uchar * key, uchar * curr_buff, uchar * father_buff,
		 uchar * father_key_pos, my_off_t father_page)
{
	my_bool		right;
	uint		k_length , father_length, father_keylength, nod_flag, curr_keylength, right_length,
			left_length  , new_right_length, new_left_length, extra_length, length,
			keys;
	...
	if ((right ? right_length : left_length) + curr_keylength <=
		...
		if (left_length < new_left_length) {	/* Move keys buff ->
							 * leaf */
