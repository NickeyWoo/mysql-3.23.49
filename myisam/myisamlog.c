...
static int 
test_if_open(struct file_info *key, element_count count,
	     struct test_if_open_param *param);
static void	fix_blob_pointers(MI_INFO * isam, byte * record);
static uint	set_maximum_open_files(uint);
static int 
test_when_accessed(struct file_info *key, element_count count,
...
static uint	verbose = 0, update = 0, test_info = 0, max_files = 0, re_open_count = 0, recover = 0,
		prefix_remove = 0, opt_processes = 0, opt_myisam_with_debug = 0;
...
static void 
get_options(register int *argc, register char ***argv)
{
	int		help      , version;
...
static int 
examine_log(my_string file_name, char **table_names)
{
	...
	while (access_time++ != number_of_commands &&
		...
		case MI_LOG_OPEN:
			if (!table_names[0]) {
				com_count[command][0]--;	/* Must be counted
								 * explicite */
				if (result)
					com_count[command][1]--;
			}
...
static int 
read_string(IO_CACHE * file, register gptr * to, register uint length)
{
...
static int 
file_info_compare(void *a, void *b)
{
	long		lint;
...
static int 
test_if_open(struct file_info *key,
...
static void 
fix_blob_pointers(MI_INFO * info, byte * record)
{
	byte           *pos;
	MI_BLOB        *blob, *end;
...
static uint 
set_maximum_open_files(uint maximum_files)
{
	...
	if (!getrlimit(RLIMIT_NOFILE, &rlimit)) {
		...
		if (setrlimit(RLIMIT_NOFILE, &rlimit)) {
			if (old_max != (int)maximum_files) {	/* Set as much as we can */
				rlimit.rlim_max = rlimit.rlim_cur = old_max;
				setrlimit(RLIMIT_NOFILE, &rlimit);
			}
		}
		getrlimit(RLIMIT_NOFILE, &rlimit);	/* Read if broken
							 * setrlimit */
...
static int 
test_when_accessed(struct file_info *key,
		   element_count count __attribute__((unused)),
		   struct st_access_param *access_param)
{
...
static void 
file_info_free(struct file_info *fileinfo)
{
...
static int 
reopen_closed_file(TREE * tree, struct file_info *fileinfo)
{
	char		name      [FN_REFLEN];
	if (close_some_file(tree))
...
static int 
find_record_with_key(struct file_info *file_info, byte * record)
{
	uint		key;
...
static bool 
cmp_filename(struct file_info *file_info, my_string name)
{
	if (!file_info)
		return 1;
