...
static int 
write_dynamic_record(MI_INFO * info, const byte * record,
		     ulong reclength);
static int 
_mi_find_writepos(MI_INFO * info, ulong reclength, my_off_t * filepos,
		  ulong * length);
static int 
update_dynamic_record(MI_INFO * info, my_off_t filepos, byte * record,
		      ulong reclength);
static int 
delete_dynamic_record(MI_INFO * info, my_off_t filepos,
		      uint second_read);
static int 
_mi_cmp_buffer(File file, const byte * buff, my_off_t filepos,
	       uint length);
...
int 
_mi_write_dynamic_record(MI_INFO * info, const byte * record)
{
...
int 
_mi_update_dynamic_record(MI_INFO * info, my_off_t pos, const byte * record)
{
...
int 
_mi_write_blob_record(MI_INFO * info, const byte * record)
{
	byte           *rec_buff;
	int		error;
...
int 
_mi_update_blob_record(MI_INFO * info, my_off_t pos, const byte * record)
{
	byte           *rec_buff;
	int		error;
	ulong		reclength, extra;
...
int 
_mi_delete_dynamic_record(MI_INFO * info)
{
...
static int 
_mi_find_writepos(MI_INFO * info,
		  ulong reclength,	/* record length */
		  my_off_t * filepos,	/* Return file pos */
		  ulong * length)
{				/* length of block at filepos */
...
static bool 
unlink_deleted_block(MI_INFO * info, MI_BLOCK_INFO * block_info)
{
		      ...
		      uint second_read)
{
	...
	do {
		...
		if (_mi_get_block_info(&del_block, info->dfile, filepos + length) &
			...
			/*
			 * We can't remove this yet as this block may be the
			 * head block
			 */
			remove_next_block = 1;
...
int 
_mi_write_part_record(MI_INFO * info,
...
{				/* *flag == 0 if header */
	...
	if (length == *reclength + 3 + long_block) {
		...
		length = *reclength + head_length;	/* Write only what is
							 * needed */
	}
			...
			if (length < reclength) {
				...
				if (block_info.filepos + block_info.block_len ==
				 ...
				 info->s->base.max_data_file_length - tmp) {
					...
					/*
					 * Check if next block is a deleted
					 * block
					 */
					MI_BLOCK_INFO	del_block;
					del_block.second_read = 0;
...
uint 
_mi_rec_pack(MI_INFO * info, register byte * to, register const byte * from)
{
	...
	for (i = info->s->base.fields; i-- > 0; from += length, rec++) {
		...
		if ((type = (enum en_fieldtype)rec->type) != FIELD_NORMAL) {
			if (type == FIELD_BLOB) {
				...
				pos = (byte *) from;
				end = (byte *) from + length;
				if (type == FIELD_SKIPP_ENDSPACE) {	/* Pack trailing spaces */
				...
				if (new_length + 1 + test(rec->length > 255 && new_length > 127)
					...
					if (rec->length > 255 && new_length > 127) {
						...
						*to++ = (char)new_length;
					memcpy((byte *) to, pos, (size_t) new_length);
					to += new_length;
					flag |= bit;
				} else {
...
/*
 * * Check if a record was correctly packed. Used only by isamchk * Returns 0
 * if record is ok.
 */

my_bool 
_mi_rec_check(MI_INFO * info, const char *record)
{
	uint		length   , new_length, flag, bit, i;
	...
	packpos = info->rec_buff;
	to = info->rec_buff + info->s->base.pack_bits;
	rec = info->s->rec;
	flag = *packpos;
	bit = 1;
...
ulong 
_mi_rec_unpack(register MI_INFO * info, register byte * to, byte * from,
	       ulong found_length)
{
...
ulong 
_my_calc_total_blob_length(MI_INFO * info, const byte * record)
{
	ulong		length;
	MI_BLOB        *blob, *end;
...
ulong 
_mi_calc_blob_length(uint length, const byte * pos)
{
	switch (length) {
	case 1:
...
void 
_my_store_blob_length(byte * pos, uint pack_length, uint length)
{
	switch (pack_length) {
	case 1:
		*pos = (uchar) length;
...
int 
_mi_read_dynamic_record(MI_INFO * info, my_off_t filepos, byte * buf)
{
	int		flag;
...
byte           *
mi_fix_rec_buff_for_blob(MI_INFO * info, ulong length)
{
	uint		extra;
...
int 
_mi_cmp_dynamic_unique(MI_INFO * info, MI_UNIQUEDEF * def,
...
int 
_mi_cmp_dynamic_record(register MI_INFO * info, register const byte * record)
{
	...
	if ((info->opt_flag & READ_CHECK_USED)) {	/* If check isn't
							 * disabled  */
		if (info->s->base.blobs) {
...
int 
_mi_read_rnd_dynamic_record(MI_INFO * info, byte * buf,
	...
	do {
		...
		if (block_info.data_len) {
			if (info->opt_flag & READ_CACHE_USED) {
				...
				/*
				 * VOID(my_seek(info->dfile,filepos,MY_SEEK_SE
				 * T,MYF(0)));
				 */
				if (my_read(info->dfile, (byte *) to, block_info.data_len, MYF(MY_NABP))) {
					if (my_errno == -1)
						my_errno = HA_ERR_WRONG_IN_RECORD;	/* Unexpected end of
											 * file */
					goto err;
				}
			}
...
uint 
_mi_get_block_info(MI_BLOCK_INFO * info, File file, my_off_t filepos)
{
	uint		return_val = 0;
