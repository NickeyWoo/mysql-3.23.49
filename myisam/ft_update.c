...
static FT_WORD *
_mi_ft_parserecord(MI_INFO * info, uint keynr, byte * keybuf,
		   const byte * record)
{
	TREE           *parsed = NULL;
	MI_KEYSEG      *keyseg;
...
static int 
_mi_ft_store(MI_INFO * info, uint keynr, byte * keybuf,
...
static int 
_mi_ft_erase(MI_INFO * info, uint keynr, byte * keybuf, FT_WORD * wlist, my_off_t filepos)
{
...
int 
_mi_ft_cmp(MI_INFO * info, uint keynr, const byte * rec1, const byte * rec2)
{
	MI_KEYSEG      *keyseg;
	byte           *pos1, *pos2;
	uint		i;
...
int 
_mi_ft_add(MI_INFO * info, uint keynr, byte * keybuf, const byte * record,
	   my_off_t pos)
{
	int		error = -1;
	FT_WORD        *wlist;
...
int 
_mi_ft_del(MI_INFO * info, uint keynr, byte * keybuf, const byte * record,
	   my_off_t pos)
{
	int		error = -1;
	FT_WORD        *wlist;
...
uint 
_ft_make_key(MI_INFO * info, uint keynr, byte * keybuf, FT_WORD * wptr,
	     my_off_t filepos)
{
