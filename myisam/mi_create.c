...
int 
mi_create(const char *name, uint keys, MI_KEYDEF * keydefs,
	  ...
	  MI_CREATE_INFO * ci, uint flags)
{
	...
	uint		fields   , length, max_key_length, packed, pointer, key_length,
			info_length  , key_segs, options, min_key_length_skipp, base_pos,
			varchar_count, long_varchar_count, varchar_length, max_key_block_length,
			unique_key_parts, offset;
	...
	for (rec = recinfo, fields = 0;
		...
		if ((type = (enum en_fieldtype)rec->type) != FIELD_NORMAL &&
			...
			if (type == FIELD_BLOB) {
				...
				packed--;
				pack_reclength += 1;
				if (test(rec->length > 257)) {	/* May be packed on 3
								 * bytes */
					long_varchar_count++;
					pack_reclength += 2;
				}
	...
	if (packed || (flags & HA_PACK_RECORD))
		options |= HA_OPTION_PACK_RECORD;	/* Must use packed
							 * records */
	...
	if (!(options & (HA_OPTION_COMPRESS_RECORD | HA_OPTION_PACK_RECORD))) {
		if (reclength <= pointer)
			reclength = pointer + 1;	/* reserve place for
							 * delete link */
	} else
		reclength += long_varchar_count;	/* We need space for
							 * this! */
	...
	for (i = 0, keydef = keydefs; i < keys; i++, keydef++) {
		...
		if (keydef->flag & HA_FULLTEXT) {	/* SerG */
			...
			if (flags & HA_DONT_TOUCH_DATA) {
				/*
				 * called by myisamchk - i.e. table structure
				 * was taken from MYI file and FULLTEXT key
				 * *do has* additional FT_SEGS keysegs. We'd
				 * better delete them now
				 */
			...
			if (keydef->flag & HA_PACK_KEY) {
				...
				/*
				 * Only use HA_PACK_KEY if the first segment
				...
				if (!(keydef->seg[0].flag & (HA_SPACE_PACK | HA_BLOB_PART |
					...
					keydef->seg[0].flag |= HA_PACK_KEY;	/* for easyer intern
										 * test */
			...
			for (j = 0, keyseg = keydef->seg; j < keydef->keysegs; j++, keyseg++) {
				/*
				 * numbers are stored with high by first to
	...
	/*
	 * max_data_file_length and max_key_file_length are recalculated on
	 * open
	 */
...
err:
	...
	case 2:
		if (!(flags & HA_DONT_TOUCH_DATA)) {
			/*
			 * QQ: Tõnu should add a call to my_raid_delete()
			 * here
			 */
			VOID(fn_format(buff, name, "", MI_NAME_DEXT, 2 + 4));
...
uint 
mi_get_pointer_length(ulonglong file_length, uint def)
{
