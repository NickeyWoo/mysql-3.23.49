...
static int 
FT_STOPWORD_cmp(FT_STOPWORD * w1, FT_STOPWORD * w2)
{
...
int 
ft_init_stopwords(const char **sws)
{
	FT_STOPWORD	sw;
	...
	for (; *sws; sws++) {
		if ((sw.len = (uint) strlen(sw.pos = *sws)) < MIN_WORD_LEN)
			continue;
...
int 
is_stopword(char *word, uint len)
{
	FT_STOPWORD	sw;
	sw.pos = word;
	sw.len = len;
