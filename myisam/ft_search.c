...
static int 
FT_SUPERDOC_cmp(FT_SUPERDOC * p1, FT_SUPERDOC * p2)
{
...
static int 
walk_and_match(FT_WORD * word, uint32 count, ALL_IN_ONE * aio)
{
...
#endif
		if (tmp_weight == 0)
			return doc_cnt;	/* stopword, doc_cnt should be 0 */
...
static int 
walk_and_copy(FT_SUPERDOC * from,
...
static int 
FT_DOC_cmp(FT_DOC * a, FT_DOC * b)
{
	return sgn(b->weight - a->weight);
}

FT_DOCLIST     *
ft_init_search(void *info, uint keynr, byte * key,
	       uint key_len, my_bool presort)
{
	...
	if (!(wtree = ft_parse(NULL, key, key_len)))
		return NULL;
...
int 
ft_read_next(FT_DOCLIST * handler, char *record)
{
