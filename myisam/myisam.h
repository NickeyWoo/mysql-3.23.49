...
#define mi_portable_sizeof_char_ptr 8
	...
	typedef struct st_mi_keydef {	/* Key definition with open & info */
		...
		uint16		underflow_block_length;	/* When to execute
							 * underflow */
		uint16		keylength;	/* Tot length of keyparts
						 * (auto) */
		uint16		minlength;	/* min length of (packed) key
						 * (auto) */
		uint16		maxlength;	/* max length of (packed) key
						 * (auto) */
...
#define MI_UNIQUE_HASH_LENGTH	4
	...
	/*
	 * Note that null markers should always be first in a row ! When
	 * creating a column, one should only specify: type, length, null_bit
	 * and null_pos
	 */
...
#define   MYISAMCHK_VERIFY 2	/* run equivalent of myisamchk -c, if
				 * corruption is detected, do myisamchk -r */
...
#define O_DATA_LOST	4
	...
	typedef struct st_sort_info {
		...
		my_off_t	pos   , max_pos, filepos, start_recpos, filelength,
				dupp         , buff_length;
		ha_rows		max_records;
	...
	typedef struct st_mi_check_param {
		...
		ulong		use_buffers, read_buffer_length, write_buffer_length, sort_buffer_length,
				sort_key_blocks;
		uint		out_flag , warning_printed, error_printed, opt_rep_quick,
				verbose;
