...
#define _sanity(a,b)
	...
	static const char *prefix_packed_txt = "packed ", *bin_packed_txt = "prefix ", *diff_txt = "stripped ", *null_txt = "NULL", *blob_txt = "BLOB ";
...
static int 
myisamchk(MI_CHECK * param, my_string filename)
{
	...
	/*
	 * Skipp the checking of the file if: We are using --fast and the
	 * table is closed properly We are using --check-only-changed-tables
	 * and the table hasn't changed
	 */
	...
	if (param->testflag & T_DESCRIPT) {
		...
		if (param->testflag & (T_REP + T_REP_BY_SORT + T_SORT_RECORDS + T_SORT_INDEX)) {
			...
			if (!error && param->testflag & T_SORT_RECORDS) {
				/*
				 * The data file is nowadays reopened in the
				 * repair code so we should soon remove the
				 * following reopen-code
				 */
#ifndef TO_BE_REMOVED
				if (param->out_flag & O_NEW_DATA) {	/* Change temp file to
									 * org file */
					...
					if (mi_open_datafile(info, info->s))
						error = 1;
					param->out_flag &= ~O_NEW_DATA;	/* We are using new
									 * datafile */
				...
				if (!error) {
					uint		key;
					/*
					 * We can't update the index in
					 * mi_sort_records if we have a
...
static void 
descript(MI_CHECK * param, register MI_INFO * info, my_string name)
{
...
static int 
mi_sort_records(MI_CHECK * param,
		register MI_INFO * info, my_string name,
		uint sort_key,
		my_bool write_info,
		my_bool update_index)
...
static int 
sort_record_index(MI_CHECK * param, MI_INFO * info, MI_KEYDEF * keyinfo,
		  ...
		  File new_file, my_bool update_index)
{
	...
	/*
	 * Clear end of block to get better compression if the table is
	 * backuped
	 */
...
void 
mi_check_print_warning(MI_CHECK * param, const char *fmt,...)
{
	va_list		args;
...
void 
mi_check_print_error(MI_CHECK * param, const char *fmt,...)
{
	va_list		args;
