...
void 
_mi_print_key(FILE * stream, register MI_KEYSEG * keyseg,
	      const uchar * key, uint length)
{
	...
	for (; keyseg->type && key < key_end; keyseg++) {
		...
		case HA_KEYTYPE_BINARY:
			if (!(keyseg->flag & HA_SPACE_PACK) && keyseg->length == 1) {	/* packed binary digit */
...
my_bool 
check_table_is_closed(const char *name, const char *where)
{
