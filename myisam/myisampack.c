...
typedef struct st_isam_mrg {
	...
	uint		free_file;
	uint		count;
	uint		min_pack_length;	/* Theese is used by packed
						 * data */
...
static void 
free_counts_and_tree_and_queue(HUFF_TREE * huff_trees,
			       uint trees,
...
static void 
check_counts(HUFF_COUNTS * huff_counts, uint trees,
	     my_off_t records);
static int 
...
static int 
save_counts_in_queue(byte * key, element_count count,
		     HUFF_TREE * tree);
static my_off_t	calc_packed_length(HUFF_COUNTS * huff_counts, uint flag);
static uint	join_same_trees(HUFF_COUNTS * huff_counts, uint trees);
static int	make_huff_decode_table(HUFF_TREE * huff_tree, uint trees);
static void 
make_traverse_code_tree(HUFF_TREE * huff_tree,
			HUFF_ELEMENT * element, uint size,
			ulong code);
static int 
write_header(PACK_MRG_INFO * isam_file, uint header_length, uint trees,
	     my_off_t tot_elements, my_off_t filelength);
static void	write_field_info(HUFF_COUNTS * counts, uint fields, uint trees);
static my_off_t	write_huff_tree(HUFF_TREE * huff_tree, uint trees);
static uint    *
make_offset_code_tree(HUFF_TREE * huff_tree,
...
static int 
save_state(MI_INFO * isam_file, PACK_MRG_INFO * mrg, my_off_t new_length,
	   ha_checksum crc);
static int 
save_state_mrg(File file, PACK_MRG_INFO * isam_file, my_off_t new_length,
	       ha_checksum crc);
...
static int	backup = 0, error_on_write = 0, test_only = 0, verbose = 0,
		silent = 0,	write_loop = 0, force_pack = 0, opt_wait = 0, isamchk_neaded = 0;
...
main(int argc, char **argv)
{
	...
	if (join_table) {	/* Join files into one */
...
static MI_INFO *
open_isam_file(char *name, int mode)
{
	MI_INFO        *isam_file;
	MYISAM_SHARE   *share;
...
static bool 
open_isam_files(PACK_MRG_INFO * mrg, char **names, uint count)
{
	uint		i        , j;
	mrg->count = 0;
	mrg->current = 0;
...
static int 
compress(PACK_MRG_INFO * mrg, char *result_table)
{
	int		error;
	...
	if (!error && !test_only) {
		char		buff      [MEMMAP_EXTRA_MARGIN];	/* End marginal for
									 * memmap */
	...
	if (!test_only) {
		...
		if (!result_table) {
			...
			isam_file->dfile = -1;	/* Tell mi_close file is
						 * closed */
		}
	}
...
static HUFF_COUNTS *
init_huff_count(MI_INFO * info, my_off_t records)
{
	reg2 uint	i;
...
static int 
get_statistic(PACK_MRG_INFO * mrg, HUFF_COUNTS * huff_counts)
{
	int		error;
	uint		length;
			...
			for (pos = record, count = huff_counts;
				...
				if (count->tree_buff) {
					...
					count->tree_buff + tree_buff_length <
						...
						if (element->count == 1) {	/* New element */
				...
				/*
				 * Save character counters and space-counts
				 * and zero-field-counts
				 */
...
/*
 * Check each tree if we should use pre-space-compress, end-space- compress,
 * empty-field-compress or zero-field-compress
 */
...
static HUFF_TREE *
make_huff_trees(HUFF_COUNTS * huff_counts, uint trees)
{
	uint		tree;
	HUFF_TREE      *huff_tree;
...
static int 
make_huff_tree(HUFF_TREE * huff_tree, HUFF_COUNTS * huff_counts)
{
	...
	if (huff_counts->tree_buff) {
		...
		while (found < 2) {	/* Our huff_trees request at least 2
					 * elements */
			new_huff_el = huff_tree->element_buffer + (found++);
			new_huff_el->count = 0;
			new_huff_el->a.leaf.null = 0;
...
static int 
compare_tree(register const uchar * s, register const uchar * t)
{
	uint		length;
...
static my_off_t 
calc_packed_length(HUFF_COUNTS * huff_counts,
...
static uint 
join_same_trees(HUFF_COUNTS * huff_counts, uint trees)
{
	uint		k        , tree_number;
...
static int 
make_huff_decode_table(HUFF_TREE * huff_tree, uint trees)
{
	uint		elements;
	for (; trees--; huff_tree++) {
...
static int 
write_header(PACK_MRG_INFO * mrg, uint head_length, uint trees,
	     my_off_t tot_elements, my_off_t filelength)
{
...
static void 
write_field_info(HUFF_COUNTS * counts, uint fields, uint trees)
{
	reg1 uint	i;
	uint		huff_tree_bits;
...
/*
 * Write all huff_trees to new datafile. Return tot count of elements in all
 * trees Returns 0 on error
 */

static my_off_t 
write_huff_tree(HUFF_TREE * huff_tree, uint trees)
{
	uint		i        , int_length;
	...
	for (elements = 0; trees--; huff_tree++) {
		...
		if (huff_tree->max_offset >= IS_OFFSET) {	/* This should be
								 * impossible */
...
static uint    *
make_offset_code_tree(HUFF_TREE * huff_tree, HUFF_ELEMENT * element,
		      uint * offset)
{
	uint           *prev_offset;
...
static int 
compress_isam_file(PACK_MRG_INFO * mrg, HUFF_COUNTS * huff_counts)
{
	int		error;
...
static char    *
make_new_name(char *new_name, char *old_name)
{
	return fn_format(new_name, old_name, "", DATA_TMP_EXT, 2 + 4);
}

static char    *
make_old_name(char *new_name, char *old_name)
{
...
static void 
init_file_buffer(File file, pbool read_buffer)
{
...
static int 
flush_buffer(ulong neaded_length)
{
	ulong		length;
...
static void 
write_bits(register ulong value, register uint bits)
{
...
static int 
save_state_mrg(File file, PACK_MRG_INFO * mrg, my_off_t new_length,
	       ha_checksum crc)
{
	MI_STATE_INFO	state;
	MI_INFO        *isam_file = mrg->file[0];
...
static int 
mrg_rrnd(PACK_MRG_INFO * info, byte * buf)
{
	int		error;
	MI_INFO        *isam_info;
	my_off_t	filepos;
