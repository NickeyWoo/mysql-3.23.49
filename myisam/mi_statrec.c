...
int 
_mi_write_static_record(MI_INFO * info, const byte * record)
{
...
int 
_mi_update_static_record(MI_INFO * info, my_off_t pos, const byte * record)
{
	info->rec_cache.seek_not_done = 1;	/* We have done a seek */
...
int 
_mi_delete_static_record(MI_INFO * info)
{
...
int 
_mi_cmp_static_record(register MI_INFO * info, register const byte * old)
{
...
int 
_mi_cmp_static_unique(MI_INFO * info, MI_UNIQUEDEF * def,
...
int 
_mi_read_static_record(register MI_INFO * info, register my_off_t pos,
...
int 
_mi_read_rnd_static_record(MI_INFO * info, byte * buf,
	...
	if (info->lock_type == F_UNLCK) {
		if (filepos >= info->state->data_file_length) {	/* Test if new records */
			...
			if ((!cache_read || share->base.reclength > cache_length) &&
			    share->r_locks == 0 && share->w_locks == 0) {	/* record not in cache */
