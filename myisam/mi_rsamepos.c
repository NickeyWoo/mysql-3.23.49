...
/*
 * * If inx >= 0 update index pointer * Returns one of the following values: *
 * 0 = Ok. * HA_ERR_KEY_NOT_FOUND = Row is deleted * HA_ERR_END_OF_FILE   =
 * End of file
 */

int 
mi_rsame_with_pos(MI_INFO * info, byte * record, int inx, my_off_t filepos)
{
	...
	if (inx >= 0) {
		info->lastkey_length = _mi_make_key(info, (uint) inx, info->lastkey, record,
						    info->lastpos);
		info->update |= HA_STATE_KEY_CHANGED;	/* Don't use
							 * indexposition */
	}
	DBUG_RETURN(0);
}				/* mi_rsame_pos */
