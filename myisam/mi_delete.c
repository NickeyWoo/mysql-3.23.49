...
static int 
d_search(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * key,
	 uint key_length, my_off_t page, uchar * anc_buff);
static int 
del(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * key, uchar * anc_buff,
    my_off_t leaf_page, uchar * leaf_buff, uchar * keypos,
    my_off_t next_block, uchar * ret_key);
static int 
underflow(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * anc_buff,
	  my_off_t leaf_page, uchar * leaf_buff, uchar * keypos);
static uint 
remove_key(MI_KEYDEF * keyinfo, uint nod_flag, uchar * keypos,
	   uchar * lastkey, uchar * page_end,
	   my_off_t * next_block);


int 
mi_delete(MI_INFO * info, const byte * record)
{
	uint		i;
	uchar          *old_key;
	int		save_errno;
	...
	for (i = 0; i < share->base.keys; i++) {
		if (((ulonglong) 1 << i) & info->s->state.key_map) {
			...
			/*
			 * The following code block is for text searching by
			 * SerG
			 */
...
int 
_mi_ck_delete(register MI_INFO * info, uint keynr, uchar * key,
	      uint key_length)
{
	int		error;
	uint		nod_flag;
...
/*
 * * Remove key below key root * Return values: * 1 if there are less
 * buffers;  In this case anc_buff is not saved * 2 if there are more buffers *
 * -1 on errors
 */

static int 
d_search(register MI_INFO * info, register MI_KEYDEF * keyinfo,
	 ...
	 uchar * anc_buff)
{
	...
	if (flag != 0) {
		if (!nod_flag) {
			...
			my_errno = HA_ERR_CRASHED;	/* This should newer
							 * happend */
			goto err;
		}
		save_flag = 0;
...
static int 
del(register MI_INFO * info, register MI_KEYDEF * keyinfo, uchar * key,
...
{				/* key before keypos in anc_buff */
	...
	my_off_t	next_page;
	uchar		keybuff  [MI_MAX_KEY_BUFF], *endpos, *next_buff, *key_start,
	               *prev_key;
	MYISAM_SHARE   *share = info->s;
	MI_KEY_PARAM	s_temp;
	DBUG_ENTER("del");
...
static int 
underflow(register MI_INFO * info, register MI_KEYDEF * keyinfo,
	  uchar * anc_buff,
...
{				/* Position to pos after key */
	int		t_length;
	uint		length   , anc_length, buff_length, leaf_length, p_length,
			s_length     , nod_flag, key_reflength, key_length;
	my_off_t	next_page;
	uchar		anc_key  [MI_MAX_KEY_BUFF], leaf_key[MI_MAX_KEY_BUFF], *buff,
	               *endpos, *next_keypos, *anc_pos, *half_pos, *temp_pos,
	               *prev_key, *after_key;
	MI_KEY_PARAM	s_temp;
	MYISAM_SHARE   *share = info->s;
	...
	if ((keypos < anc_buff + anc_length && (share->rnd++ & 1)) ||
	    keypos == anc_buff + 2 + key_reflength) {	/* Use page right of
							 * anc-page */
		...
		if (keyinfo->flag & HA_BINARY_PACK_KEY) {
			...
			buff[0] = buff[1] = 0;	/* Avoid length error check
						 * if packed key */
...
/*
 * remove a key from packed buffert The current code doesn't handle the case
 * that the next key may be packed better against the previous key if there
 * is a case difference returns how many chars was removed or 0 on error
 */

static uint 
remove_key(MI_KEYDEF * keyinfo, uint nod_flag,
		...
		if (keypos != page_end) {
			if (keyinfo->flag & HA_BINARY_PACK_KEY) {
				...
				if (next_length > prev_length) {
					/*
					 * We have to copy data from the
					 * current key to the next key
					 */
				...
				if ((keyinfo->seg->flag & HA_PACK_KEY) && *keypos & 128) {
					/*
					 * Next key is packed against the
					 * current one
					 */
					uint		next_length, prev_length,
							prev_pack_length, lastkey_length, rest_length;
					...
					if (next_length > prev_length) {	/* Key after is based on
										 * deleted key */
