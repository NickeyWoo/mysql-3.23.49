...
#undef my_write			/* undef map from my_nosys; We need
				 * test-if-disk full */
#endif
...
typedef struct st_mi_state_info {
	struct {		/* Fileheader */
		...
		uchar		unique_key_parts[2];	/* Key parts + unique
							 * parts */
		uchar		keys;	/* number of keys in file */
		uchar		uniques;	/* number of UNIQUE
						 * definitions */
	...
	my_off_t	rec_per_key_rows;	/* Rows when calculating
						 * rec_per_key */
...
typedef struct st_mi_isam_share {	/* Shared between opens */
	...
	my_bool		changed,/* If changed since lock */
			...
			not_flushed  , temporary, delay_key_write, concurrent_insert, fulltext_index;
	myf		write_flag;
	int		rnd;	/* rnd-counter */
...
struct st_myisam_info {
	...
	ha_checksum	checksum;
	ulong		packed_length, blob_length;	/* Length of found,
							 * packed record */
	uint		alloced_rec_buff_length;	/* Max recordlength
							 * malloced */
	...
	my_off_t	last_search_keypage;	/* Last keypage when
						 * searching */
	...
	my_bool		quick_mode;
	my_bool		page_changed;	/* If info->buff can't be used for
					 * rnext */
	my_bool		buff_used;	/* If info->buff has to be reread for
					 * rnext */
...
#define USE_WHOLE_KEY	MI_MAX_KEY_BUFF*2	/* Use whole key in
						 * _mi_search() */
...
typedef struct st_mi_s_param {
	uint		ref_length, key_length, n_ref_length, n_length, totlength, part_of_prev_key,
			prev_length  , pack_marker;
...
extern int 
_mi_insert(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * key,
	   uchar * anc_buff, uchar * key_pos, uchar * key_buff,
	   uchar * father_buff, uchar * father_keypos,
	   my_off_t father_page, my_bool insert_last);
extern int 
_mi_split_page(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * key,
	       uchar * buff, uchar * key_buff, my_bool insert_last);
extern uchar   *
_mi_find_half_pos(uint nod_flag, MI_KEYDEF * keyinfo, uchar * page,
...
_mi_calc_var_key_length(MI_KEYDEF * keyinfo, uint nod_flag,
			...
			uchar * key, MI_KEY_PARAM * s_temp);
extern int 
_mi_calc_var_pack_key_length(MI_KEYDEF * keyinfo, uint nod_flag,
			     uchar * key_pos, uchar * org_key,
			     uchar * prev_key,
			     uchar * key, MI_KEY_PARAM * s_temp);
extern int 
_mi_calc_bin_pack_key_length(MI_KEYDEF * keyinfo, uint nod_flag,
			     uchar * key_pos, uchar * org_key,
			     uchar * prev_key,
			     uchar * key, MI_KEY_PARAM * s_temp);
void 
_mi_store_static_key(MI_KEYDEF * keyinfo, uchar * key_pos,
		     MI_KEY_PARAM * s_temp);
void 
_mi_store_var_pack_key(MI_KEYDEF * keyinfo, uchar * key_pos,
		       MI_KEY_PARAM * s_temp);
#ifdef NOT_USED
void 
_mi_store_pack_key(MI_KEYDEF * keyinfo, uchar * key_pos,
		   MI_KEY_PARAM * s_temp);
#endif
void 
_mi_store_bin_pack_key(MI_KEYDEF * keyinfo, uchar * key_pos,
		       MI_KEY_PARAM * s_temp);
...
extern int 
_mi_search(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * key, uint key_len,
	   uint nextflag, my_off_t pos);
extern int 
_mi_bin_search(struct st_myisam_info *info, MI_KEYDEF * keyinfo,
	       uchar * page, uchar * key, uint key_len, uint comp_flag,
	       uchar * *ret_pos, uchar * buff, my_bool * was_last_key);
extern int 
_mi_seq_search(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * page,
	       uchar * key, uint key_len, uint comp_flag,
	       uchar ** ret_pos, uchar * buff, my_bool * was_last_key);
extern int 
_mi_compare_text(CHARSET_INFO *, uchar *, uint, uchar *, uint,
		 my_bool);
...
extern int 
_mi_key_cmp(MI_KEYSEG * keyseg, uchar * a, uchar * b,
	    uint key_length, uint nextflag, uint * diff_length);
extern uint 
_mi_get_static_key(MI_KEYDEF * keyinfo, uint nod_flag, uchar * *page,
		   uchar * key);
extern uint 
_mi_get_pack_key(MI_KEYDEF * keyinfo, uint nod_flag, uchar * *page,
		 uchar * key);
extern uint 
_mi_get_binary_pack_key(MI_KEYDEF * keyinfo, uint nod_flag,
			uchar ** page_pos, uchar * key);
extern uchar   *
_mi_get_last_key(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * keypos,
...
extern int 
_mi_search_next(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * key,
...
extern uchar   *
_mi_fetch_keypage(MI_INFO * info, MI_KEYDEF * keyinfo, my_off_t page,
		  uchar * buff, int return_buffer);
extern int 
_mi_write_keypage(MI_INFO * info, MI_KEYDEF * keyinfo, my_off_t page,
		  uchar * buff);
extern int	_mi_dispose(MI_INFO * info, MI_KEYDEF * keyinfo, my_off_t pos);
extern my_off_t	_mi_new(MI_INFO * info, MI_KEYDEF * keyinfo);
extern uint 
_mi_make_key(MI_INFO * info, uint keynr, uchar * key,
	     const byte * record, my_off_t filepos);
extern uint 
_mi_pack_key(MI_INFO * info, uint keynr, uchar * key, uchar * old,
	     uint key_length);
extern int	_mi_read_key_record(MI_INFO * info, my_off_t filepos, byte * buf);
extern int 
_mi_read_cache(IO_CACHE * info, byte * buff, my_off_t pos,
...
extern ulong 
_mi_rec_unpack(MI_INFO * info, byte * to, byte * from,
	       ulong reclength);
extern my_bool	_mi_rec_check(MI_INFO * info, const char *from);
extern int 
_mi_write_part_record(MI_INFO * info, my_off_t filepos, ulong length,
		      my_off_t next_filepos, byte ** record,
		      ulong * reclength, int *flag);
extern void 
_mi_print_key(FILE * stream, MI_KEYSEG * keyseg, const uchar * key,
	      uint length);
...
extern int 
_mi_pack_rec_unpack(MI_INFO * info, byte * to, byte * from,
		    ulong reclength);
