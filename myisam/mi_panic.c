...
/*
 * if flag == HA_PANIC_WRITE then all misam files are unlocked and all
 * changed data in single user misam is written to file
 */
/*
 * if flag == HA_PANIC_READ then all misam files that was locked when
 * mi_panic(HA_PANIC_WRITE) was done is locked. A mi_readinfo() is done for
 * all single user files to get changes in database
 */


int 
mi_panic(enum ha_panic_function flag)
{
	int		error = 0;
	...
	for (list_element = myisam_open_list; list_element; list_element = next_open) {
		...
		case HA_PANIC_WRITE:	/* Do this to free databases */
			...
			if (info->dfile >= 0 && my_close(info->dfile, MYF(0)))
				error = my_errno;
			info->s->kfile = info->dfile = -1;	/* Files aren't open
								 * anymore */
			break;
#endif
