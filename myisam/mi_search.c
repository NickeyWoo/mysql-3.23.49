...
static my_bool 
_mi_get_prev_key(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * page,
...
int 
_mi_check_index(MI_INFO * info, int inx)
{
...
/*
 * * Search after row by a key * Position to row is stored in info->lastpos *
 * Return: -1 if not found *	    1 if one should continue search on higher
 * level
 */

int 
_mi_search(register MI_INFO * info, register MI_KEYDEF * keyinfo,
...
int 
_mi_bin_search(MI_INFO * info, register MI_KEYDEF * keyinfo, uchar * page,
...
int 
_mi_seq_search(MI_INFO * info, register MI_KEYDEF * keyinfo, uchar * page,
	       uchar * key, uint key_len, uint comp_flag, uchar ** ret_pos,
...
my_off_t 
_mi_kpos(uint nod_flag, uchar * after_key)
{
	after_key -= nod_flag;
	switch (nod_flag) {
#if SIZEOF_OFF_T > 4
...
void 
_mi_kpointer(register MI_INFO * info, register uchar * buff, my_off_t pos)
{
...
my_off_t 
_mi_dpos(MI_INFO * info, uint nod_flag, uchar * after_key)
{
	my_off_t	pos;
...
my_off_t 
_mi_rec_pos(MYISAM_SHARE * s, uchar * ptr)
{
	my_off_t	pos;
...
void 
_mi_dpointer(MI_INFO * info, uchar * buff, my_off_t pos)
{
	if (!(info->s->options &
...
int 
_mi_compare_text(CHARSET_INFO * charset_info, uchar * a, uint a_length,
...
static int 
compare_bin(uchar * a, uint a_length, uchar * b, uint b_length,
	    my_bool part_key)
{
	uint		length = min(a_length, b_length);
	uchar          *end = a + length;
...
/*
 * * Compare two keys with is bigger * Returns <0, 0, >0 acording to with is
 * bigger * Key_length specifies length of key to use.  Number-keys can't *
 * be splited * If flag <> SEARCH_FIND compare also position
 */

#define FCMP(A,B) ((int) (A) - (int) (B))

int 
_mi_key_cmp(register MI_KEYSEG * keyseg, register uchar * a,
		...
		case HA_KEYTYPE_NUM:	/* Numeric key */
			{
				...
				if (keyseg->flag & HA_REVERSE_SORT) {
					swap(uchar *, a, b);
					swap_flag = 1;	/* Remember swap of a &
							 * b */
					end = a + (int)(end - b);
				}
				if (keyseg->flag & HA_SPACE_PACK) {
...
uint 
_mi_get_static_key(register MI_KEYDEF * keyinfo, uint nod_flag,
...
uint 
_mi_get_pack_key(register MI_KEYDEF * keyinfo, uint nod_flag,
	...
	for (keyseg = keyinfo->seg; keyseg->type; keyseg++) {
		if (keyseg->flag & HA_PACK_KEY) {
			...
			if (packed) {
				...
				/*
				 * If the stored length has changed, we must
				 * move the key
				 */
				if (tot_length >= 255 && *start != 255) {
					/*
					 * length prefix changed from a
					 * length of one to a length of 3
					 */
...
uint 
_mi_get_binary_pack_key(register MI_KEYDEF * keyinfo, uint nod_flag,
			register uchar ** page_pos, register uchar * key)
{
	reg1 MI_KEYSEG *keyseg;
	uchar          *start_key, *page = *page_pos, *page_end, *from,
	               *from_end;
	uint		length   , tmp;
	...
	if (length) {
		...
		from = page;
		from_end = page_end;	/* Not packed key */
	}

	/*
	 * The trouble is that key is split in two parts: The first part is
	 * in from ...from_end-1. The second part starts at page
	 */
	for (keyseg = keyinfo->seg; keyseg->type; keyseg++) {
		if (keyseg->flag & HA_NULL_PART) {
			if (from == from_end) {
...
uchar          *
_mi_get_key(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * page,
...
uchar          *
_mi_get_last_key(MI_INFO * info, MI_KEYDEF * keyinfo, uchar * page,
...
uint 
_mi_keylength(MI_KEYDEF * keyinfo, register uchar * key)
{
	reg1 MI_KEYSEG *keyseg;
	uchar          *start;
...
uchar          *
_mi_move_key(MI_KEYDEF * keyinfo, uchar * to, uchar * from)
{
	reg1 uint	length;
...
int 
_mi_search_next(register MI_INFO * info, register MI_KEYDEF * keyinfo,
		uchar * key, uint key_length, uint nextflag, my_off_t pos)
{
	 ...
	 * and the key tree has changed since we used it last time Note that
	 * even if the key tree has changed since last read, we can use the
	 * last read data from the leaf if we haven't used the buffer for
	 * something else.
	 */
...
int 
_mi_search_first(register MI_INFO * info, register MI_KEYDEF * keyinfo,
...
int 
_mi_search_last(register MI_INFO * info, register MI_KEYDEF * keyinfo,
		register my_off_t pos)
{
	uint		nod_flag;
	uchar          *buff, *page;
 ...
 * 
 * If the max length of first key segment <= 127 characters the prefix is 1 byte
 * else it's 2 byte
 * 
 * prefix byte	 The high bit is set if this is a prefix for the prev key
 * length	 Packed length if the previous was a prefix byte [length]
 * ength character of data next-key-seg	 Next key segments
 * 
 * If the first segment can have NULL: The length is 0 for NULLS and 1+length
			     ...
			     MI_KEY_PARAM * s_temp)
{
	reg1 MI_KEYSEG *keyseg;
	int		length;
	uint		key_length, ref_length, org_key_length = 0, length_pack,
			new_key_length, diff_flag, pack_marker;
	...
	/*
	 * Calc how many characters are identical between this and the prev.
	 * key
	 */
	if (prev_key) {
	...
	if (same_length && key == key_end) {
		...
		if (next_key) {	/* Can't combine with next */
			s_temp->n_length = *next_key;	/* Needed by
							 * _mi_store_key */
			next_key = 0;
		}
	} else {
	...
	if ((s_temp->next_key_pos = next_key)) {
		...
		if (n_length || packed) {	/* Don't pack 0 length keys */
			...
			if (packed) {
				/*
				 * If first key and next key is packed (only
				 * on delete)
				 */
				if (!prev_key && org_key) {
				...
				if (!n_length) {
					/*
					 * We put a different key between two
					 * identical variable length keys
					 * Extend next key to have same
					 * prefix as this key
					 */
					if (new_ref_length) {	/* prefix of previus key *//* m
								 * ake next key longer */
				...
				/*
				 * Test if new keys has fewer characters that
				 * match the previous key
				 */
				...
				if (ref_length + pack_marker > new_ref_length) {
					...
					/*
					 * We must copy characters from the
					 * original key to the next key
					 */
					s_temp->part_of_prev_key = new_ref_length;
...

			{
				...
				if (key + n_length < key_end)	/* Normalize length
								 * based */
				...
				if (!(tmp_length = (uint) (key - start))) {	/* Key can't be
										 * re-packed */
			     ...
			     MI_KEY_PARAM * s_temp)
{
	...
	if (prev_key) {		/* If not first key in block */
		...
		/*
		 * As keys may be identical when running a sort in myisamchk,
		 * we have to guard against the case where keys may be
		 * identical
		 */
		uchar          *end;
		end = key + key_length;
	...
	if ((s_temp->next_key_pos = next_key)) {	/* If another key after */
		...
		if (next_length > ref_length) {
			/*
			 * We put a key with different case between two keys
			 * with the same prefix Extend next key to have same
			 * prefix as this key
			 */
...
void		_mi_store_var_pack_key(MI_KEYDEF * keyinfo __attribute__((unused)),
	...
	if (s_temp->prev_length) {
		/*
		 * Extend next key because new key didn't have same prefix as
		 * prev key
		 */
