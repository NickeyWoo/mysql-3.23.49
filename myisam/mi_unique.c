...
my_bool 
mi_check_unique(MI_INFO * info, MI_UNIQUEDEF * def, byte * record,
...
ha_checksum 
mi_unique_hash(MI_UNIQUEDEF * def, const byte * record)
{
	const byte     *pos, *end;
	ha_checksum	crc = 0;
	MI_KEYSEG      *keyseg;
...
int 
mi_unique_comp(MI_UNIQUEDEF * def, const byte * a, const byte * b,
