...
static void 
copy_key(struct st_myisam_info *info, uint inx,
	 uchar * record, uchar * key);

static int	verbose = 0, testflag = 0, first_key = 0, async_io = 0, key_cacheing = 0,
		write_cacheing = 0, locking = 0, rec_pointer_size = 0, pack_fields = 1,
		use_log = 0,	silent = 0, opt_quick_mode = 0;
static int	pack_seg = HA_SPACE_PACK, pack_type = HA_PACK_KEY, remove_count = -1, create_flag = 0;
...
static char	record[300], record2[300], key[100], key2[100], read_record[300],
		read_record2  [300], read_record3[300];
...
main(int argc, char *argv[])
{
	...
	uint		write_count, update, dupp_keys, opt_delete, start, length,
			blob_pos     , reclength, ant, found_parts;
	my_off_t	lastpos;
	...
	while (mi_rprev(file, read_record3, 0) == 0 &&
	       bcmp(read_record3 + start, key, length) == 0)
		ant++;
	if (ant != dupp_keys) {
	...
	for (i = strlen(key2); i-- > 1;) {
		...
		/*
		 * The following row is just to catch some bugs in the key
		 * code
		 */
	...
	if (dupp_keys > 2) {
		...
		if (mi_rkey(file, read_record, 0, key, 0, HA_READ_KEY_EXACT))
			goto err;
		if (mi_rnext(file, read_record3, 0))
			goto err;
		if (mi_delete(file, read_record3))
			goto err;
		opt_delete++;
		ant = 1;
	...
	if (dupp_keys > 4) {
		...
		if (mi_rprev(file, read_record3, 0))
			goto err;
		if (mi_rprev(file, read_record3, 0))
			goto err;
		if (mi_delete(file, read_record3))
	...
	if (dupp_keys > 6) {
		...
		if (mi_rkey(file, read_record3, 0, key, 0, HA_READ_KEY_EXACT))
			goto err;
		if (mi_delete(file, read_record3))
			goto err;
		opt_delete++;
	...
	for (i = min(2, keys); i-- > 0;) {
		if (mi_rsame(file, read_record2, (int)i))
			goto err;
...
static void 
fix_length(byte * rec, uint length)
{
...
static void 
put_blob_in_record(char *blob_pos, char **blob_buffer)
{
	ulong		i       , length;
	if (use_blob) {
		if (rnd(10) == 0) {
...
static void 
copy_key(MI_INFO * info, uint inx, uchar * rec, uchar * key_buff)
{
	MI_KEYSEG      *keyseg;
